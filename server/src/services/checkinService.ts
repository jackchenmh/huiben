...
export class CheckInService {
  ...
  async createCheckIn(checkInData: Omit<CheckIn, 'id' | 'createdAt' | 'updatedAt'>): Promise<CheckIn> {
    ...
    // 检查并授予徽章，获取新徽章列表
    const newBadges = await this.gameService.checkAndAwardBadges(checkInData.userId);

    // 检查是否升级
    const levelResult = await this.gameService.updateLevel(checkInData.userId);
    ...
    if (levelResult.levelUp) {
      await this.notificationService.sendAchievementNotification(
        checkInData.userId,
        'level_up',
        `等级 ${levelResult.newLevel}`
      );
    }

    // 检查连续阅读并发送相关通知
    const userStats = await this.gameService.getUserStats(checkInData.userId);
    if (userStats && userStats.consecutiveDays > 0 && userStats.consecutiveDays % 7 === 0) {
      await this.notificationService.sendAchievementNotification(
        checkInData.userId,
        'streak',
        userStats.consecutiveDays.toString()
      );
    }

    // 检查每日挑战完成情况
    await this.gameService.completeDailyChallenge(checkInData.userId);
