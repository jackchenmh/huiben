...
export class BookService {
  ...
  async getBookStats(bookId: number): Promise<{ totalCheckins: number; participants: number; avgReadingTime: number }> {
    const totals = await db.get(
      `SELECT COUNT(*) as totalCheckins, AVG(readingTime) as avgReadingTime
       FROM checkins WHERE bookId = ?`,
      [bookId]
    );

    const participantsRow = await db.get(
      `SELECT COUNT(DISTINCT userId) as participants FROM checkins WHERE bookId = ?`,
      [bookId]
    );

    return {
      totalCheckins: totals?.totalCheckins || 0,
      participants: participantsRow?.participants || 0,
      avgReadingTime: Math.round(totals?.avgReadingTime || 0),
    };
  }
}
